...
# Override dataset settings - wholebody_subset 사용
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root=data_root,
        ann_file='wholebody_subset/wholebody_train.json',
        data_prefix=dict(img='wholebody_subset/train/'),
        pipeline=[
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        metainfo=dict(classes=classes),
        data_root=data_root,
        ann_file='wholebody_subset/wholebody_val.json',
        data_prefix=dict(img='wholebody_subset/val/'),
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='Resize', scale=(1024, 1024), keep_ratio=True),
            dict(type='Pad', size=(1024, 1024), pad_val=dict(img=(114, 114, 114))),
...
test_dataloader = dict(
    ...
    dataset=dict(
        ...
        metainfo=dict(classes=classes),
        data_root=data_root,
        ann_file='wholebody_subset/wholebody_test.json',
        data_prefix=dict(img='wholebody_subset/test/'),
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='Resize', scale=(1024, 1024), keep_ratio=True),
            dict(type='Pad', size=(1024, 1024), pad_val=dict(img=(114, 114, 114))),
...
val_evaluator = dict(
    ann_file=data_root + 'wholebody_subset/wholebody_val.json',
    metric='bbox'
)

test_evaluator = dict(
    ann_file=data_root + 'wholebody_subset/wholebody_test.json',
    metric='bbox',
    classwise=True
)
